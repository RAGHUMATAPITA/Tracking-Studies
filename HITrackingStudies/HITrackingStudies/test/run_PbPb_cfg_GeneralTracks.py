...
process.TFileService = cms.Service("TFileService",
    fileName = cms.string('/eos/cms/store/group/phys_heavyions/rpradhan/TrackingStudies/2018PbPb_MAODData_GeneralTracks.root')
)
...
if (options.sample == "Data_Reco_AOD" or options.sample == "Data_MiniAOD"):
    #process.GlobalTag = GlobalTag(process.GlobalTag, '124X_dataRun3_Prompt_v10', '') # for Run3
    process.GlobalTag = GlobalTag(process.GlobalTag, '112X_dataRun2_PromptLike_HI_v3', '') # For Run 2
###
...
if (options.sample == "MC_Reco_AOD" or options.sample == "MC_MiniAOD" or options.sample == "Data_Reco_AOD" or options.sample == "Data_MiniAOD"):
    ...
    if (options.sample == "MC_MiniAOD" or options.sample == "Data_MiniAOD"):
        ...
        process.anaTrack.packedCandSrc = cms.InputTag('packedPFCandidates') # for general tracks,  added by raghu
        #process.anaTrack.packedCandSrc = cms.InputTag('hiPixelTracks') # for pixel tracks,  added by raghu
        process.anaTrack.isPixelTrack = False # turn on for pixel tracking only, added by raghu
