///Please, run it by doing: <<root -l -b -q macro_control_plots_TwoSamples.C>>
...
///Please, change name of input files below variables "fileName1" and "fileName2"
...
int cent_max = 100;
//input file
auto fileName1 = "/eos/cms/store/group/phys_heavyions/rpradhan/TrackingStudies/2018PbPb_MAODData_GeneralTracks.root"; //ratio plots are fileName1/fileName2
auto fileName2 = "/eos/cms/store/group/phys_heavyions/rpradhan/TrackingStudies/2018PbPb_MAODData_PixelTracks.root";
...
void macro_control_plots_TwoSamples(){
...
gSystem->Exec("mkdir -p ControlPlotsTwoSamples"); //Directory to save the histograms
gStyle->SetOptStat(0);
...
for (unsigned int i=0; i<N_variables; i++){
   ...
   legendStyle(leg_norm, h_all1.GetPtr(), h_all2.GetPtr());  //call function to customize style of TCanvas
   leg_norm->Draw();
   gPad->Update();     
   ...
   gPad->SetTickx(1);
   gPad->SetTicky(1);
   TH1D *h_ratio = (TH1D*)h_all1->Clone(hist_name_all[i]+"_ratio");
   h_ratio->Divide(h_all2.GetPtr());
   h_ratio->Draw();
